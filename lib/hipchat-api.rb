...
module HipChat
  class API
    ...
    def rooms_create(name, owner_user_id, privacy = 'public', topic = '', guest_access = 0)
      self.class.post(hipchat_api_url_for('rooms/create'), :query => {:auth_token => @token, :name => name, :owner_user_id => owner_user_id, 
    ...
    def rooms_delete(room_id)
      self.class.post(hipchat_api_url_for('rooms/delete'), :query => {:auth_token => @token, :room_id => room_id})      
    end
    ...
    def rooms_message(room_id, from, message, notify = 0, color = 'yellow')
      self.class.post(hipchat_api_url_for('rooms/message'), :query => {:auth_token => @token, :auth_token => @token, :room_id => room_id, :from => from, :message => message, :notify => notify, :color => color})
    end
    ...
    def users_create(email, name, title, is_group_admin = 0, password = nil, timezone = 'UTC')
      self.class.post(hipchat_api_url_for('users/create'), :query => {:auth_token => @token, :email => email, :name => name, :title => title, :is_group_admin => is_group_admin, :password => password, :timezone => timezone}.reject!{|key, value| value.nil?})
    end
    ...
    def users_delete(user_id)
      self.class.post(hipchat_api_url_for('users/delete'), :query => {:auth_token => @token, :user_id => user_id})
    end
    ...
    def users_update(user_id, email = nil, name = nil, title = nil, is_group_admin = nil, password = nil, timezone = nil)
      self.class.post(hipchat_api_url_for('users/update'), :query => {:auth_token => @token, :user_id => user_id, :email => email, 
